        ...
        success:function(result){
            ...
            if(result.status.name == "ok"){
                 ...
                 // $('#temp').html(result['data']['main']['temp']);
                  ...
                  $('#sunrise_id').html(new Date(result['data']['sys']['sunrise']*1000));
                  $('#sunset_id').html(new Date(result['data']['sys']['sunset']*1000));

                  let temp = `${Math.round(result.data.main.temp)}°C`;
                  $('#temp').html(`${temp}`);
                  let feelsLike = `${Math.round(result.data.main.feels_like)}°C`;
